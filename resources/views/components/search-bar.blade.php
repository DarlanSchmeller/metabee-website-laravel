<form method="GET" action="{{ route('cursos.search') }}">
    <div class="max-w-2xl mx-auto mb-8">
        <div class="relative">
            ...
            <input type="text" name="keywords" placeholder="Buscar cursos..." value="{{ request('keywords') }}"
    ...
    <div class="flex flex-wrap justify-center gap-3">
        @foreach ($categories as $cat)
            <label class="relative">
                ...
                <input onchange="this.form.submit()" type="checkbox" name="categories[]" value="{{ $cat['label'] }}"
                    class="peer hidden"
                    {{ in_array($cat['label'], (array) request('categories', [])) ? 'checked' : '' }} />

                {{-- Styled content --}}
                <div
                    class="flex items-center space-x-2 px-4 py-2 rounded-xl font-medium cursor-pointer transition-all duration-300 border
        border-amber-500/20 bg-gray-900/50 text-gray-300 hover:border-amber-500/40
        peer-checked:bg-gradient-to-r peer-checked:from-amber-500 peer-checked:to-yellow-500 peer-checked:text-gray-900
        peer-checked:shadow-lg peer-checked:shadow-amber-500/30">
