        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-8">
            @foreach ($stats as $stat)
                <div class="text-center group">
                    <div
                        class="inline-flex items-center justify-center w-16 h-16 {{ $stat["bgColor"] }} border {{ $stat["borderColor"] }} rounded-2xl mb-4 group-hover:scale-110 group-hover:shadow-lg group-hover:shadow-amber-500/20 transition-all duration-300"
                    >
                        @if ($stat["icon"] === "users")
