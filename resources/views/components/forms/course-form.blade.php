...
<form method="POST" action="{{ !empty($course) ? route('cursos.update', $course->id) : route('cursos.store') }}"
    ...
    <div x-show="step === 1" x-transition>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <x-select-input label="Categoria" name="category" :options="$courseCategories" :selected="$course?->category" />

            <x-text-area-input label="Descrição curta" name="description" rows="3"
                placeholder="Digite uma descrição curta" required :value="old('description', $course?->description ?? '')" />

            <x-text-area-input label="Descrição completa" name="fullDescription" rows="5"
                placeholder="Digite a descrição completa do curso" :value="old('fullDescription', $course?->fullDescription ?? '')" />
        </div>
    </div>
    ...
    <div x-show="step === 2" x-transition>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            @foreach ($courseFields as [$label, $name, $type, $placeholder, $step])
                <x-form-input :label="$label" :name="$name" :type="$type" :placeholder="$placeholder" :min="$type === 'number' ? $placeholder : null"
                    :step="$step" :value="old($name, $course?->$name ?? '')" />
            @endforeach

            <x-select-input label="Nível" name="level" :options="[
                'iniciante' => 'Iniciante',
        ...
        <div x-data="curriculumBuilder(
            ...
            <template x-for="(module, moduleIndex) in modules" :key="moduleIndex">
                <div class="bg-gray-900/40 border border-gray-700 rounded-xl p-5 relative space-y-4">
                    <!-- Remove module -->
                    <button type="button" class="absolute top-3 right-3 text-red-400 hover:text-red-500"
                        @click="removeModule(moduleIndex)" title="Remover módulo">
                    ...
                    <!-- Module -->
                    <input type="text" :name="'modules[' + moduleIndex + '][title]'" placeholder="Título do módulo"
                    ...
                    <template x-for="(lesson, lessonIndex) in module.lessons" :key="lessonIndex">
                        <div class="bg-gray-800/60 border border-gray-700 rounded-lg p-3 relative">
                            ...
                            <button type="button" class="absolute top-2 right-2 text-red-400 hover:text-red-500"
                                @click="removeLesson(module, lessonIndex)" title="Remover aula">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                                <div>
                                    ...
                                    <template x-if="getLessonError(moduleIndex, lessonIndex, 'title')">
                                        <p class="mt-1 text-sm text-red-500"
                                            x-text="getLessonError(moduleIndex, lessonIndex, 'title')"></p>
                                    </template>
                                </div>
                                ...
                                <div>
                                    ...
                                    <template x-if="getLessonError(moduleIndex, lessonIndex, 'url')">
                                        <p class="mt-1 text-sm text-red-500"
                                            x-text="getLessonError(moduleIndex, lessonIndex, 'url')"></p>
                                    </template>
                                </div>
                                ...
                                <div>
                                    ...
                                    <template x-if="getLessonError(moduleIndex, lessonIndex, 'duration')">
                                        <p class="mt-1 text-sm text-red-500"
                                            x-text="getLessonError(moduleIndex, lessonIndex, 'duration')"></p>
                                    </template>
                                </div>
                            </div>
                        </div>
    ...
    <div x-show="step === 4" x-transition>
        ...
        <div class="space-y-8">
            ...
            <div class="grid sm:grid-cols-2 gap-6">
                <x-checkbox-input name="certificate" label="Oferece certificado" :checked="old('certificate', $course?->certificate ?? '')" />
                <x-checkbox-input name="resources" label="Inclui materiais de apoio" :checked="old('resources', $course?->resources ?? '')" />
            </div>

            <x-text-input label="Tags (separadas por vírgula)" name="tags"
                placeholder="Ex: JavaScript, Web, Backend" :value="old(
                    'tags',
                    is_array($course?->tags ?? null) ? implode(', ', $course->tags) : $course?->tags ?? '',
                )" />

            <x-text-area-input label="O que você vai aprender (separado por vírgula)" name="whatYouLearn"
                rows="3" placeholder="Digite cada item separado por vírgula" :value="old(
                    'whatYouLearn',
            ...
            <x-text-area-input label="Skills (separadas por vírgula)" name="skills" rows="3"
                placeholder="Ex: JavaScript, Laravel, Tailwind" :value="old(
                    'skills',
                    is_array($course?->skills ?? null) ? implode(', ', $course->skills) : $course?->skills ?? '',
                )" />

            <x-text-area-input label="Requisitos (separados por vírgula)" name="requirements" rows="3"
                placeholder="Ex: Conhecimento básico de HTML/CSS" :value="old(
                    'requirements',
