...
<form action="{{ route('account.credentials.update') }}" method="POST"
    ...
    <div class="grid gap-4 sm:grid-cols-2">
        ...
        <div class="space-y-2">
            ...
            <div class="relative">
                <x-heroicon-o-lock-closed class="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-500" />
                <input :type="showPassword ? 'text' : 'password'" id="new-password" name="new-password" required
                    placeholder="••••••••"
                    class="pl-10 pr-10 bg-zinc-800/50 border outline-none border-zinc-700 text-white placeholder:text-gray-300 focus:border-amber-500 focus:ring-amber-500/20 focus:ring-1 rounded-lg w-full py-3 text-base md:text-lg transition" />
                @error('new-password')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
        ...
        <div class="space-y-2">
            ...
            <div class="relative">
                <x-heroicon-o-lock-closed class="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-500" />
                <input :type="showPassword ? 'text' : 'password'" id="new-password_confirmation"
                    name="new-password_confirmation" required placeholder="Confirme sua senha"
                    class="pl-10 pr-10 bg-zinc-800/50 border outline-none border-zinc-700 text-white placeholder:text-gray-300 focus:border-amber-500 focus:ring-amber-500/20 focus:ring-1 rounded-lg w-full py-3 text-base md:text-lg transition" />
