...
<nav class="flex justify-center space-x-2" role="navigation">
    ...
    @foreach ($elements as $element)
        @if (is_string($element))
            <span class="px-4 py-2 bg-gray-800 text-gray-500 cursor-default">{{ $element }}</span>
        @endif

        @if (is_array($element))
            @foreach ($element as $page => $url)
                @if ($page == $paginator->currentPage())
                    <span class="px-4 py-2 bg-gradient-to-r from-amber-500 to-yellow-500 text-gray-900 font-semibold rounded-xl shadow-md shadow-amber-500/30">
                        {{ $page }}
                    </span>
                @else
                    <a href="{{ $url }}"
